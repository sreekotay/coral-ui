...
RS.getOffset = function(e) {
	var t = e.target ? e.target : e.srcElement,
			n = 0;
	while (t && !isNaN(t.offsetLeft) && !isNaN(t.offsetTop)) {
			n += t.offsetTop - t.scrollTop;
			t = t.offsetParent
	}
	n = e.clientY + RS.getScrollTop() - n;
	return n
};
...
RS.originalOnselectstart = !1;
RS.enableSelection = function(e) {
	if (e) window.document.onselectstart = RS.originalOnselectstart;
	else {
...
RS.detectIE = function() {
	return document.all ? !0 : !1
};
var RS = RS || {};
RS.__id = 0;
RS.RocketScroll = function(e, t) {
	if (RS.detectTouchScreen()) return;
	t = t || !1;
	t ? this.el = e : this.el = RS.$(e);
	if (typeof this.el.length != "undefined" && this.el.length > 1) {
			this.elements = [];
			for (var n = 0; n < this.el.length; n++) this.elements.push(new RS.RocketScroll(this.el.item(n), !0));
			this.multiple = !0;
			return
	}
	this.multiple = !1;
	this.el.id || (this.el.id = "rocketScroll" + RS.__id++);
...
RS.RocketScroll.prototype.bindEvents = function() {
	...
	this.scrollDiv.onscroll = function() {
			e.handle.style.marginTop = e.ratio * this.scrollTop + "px"
	};
	this.handle.onclick = function(e) {
			RS.stopPropagation(e)
	};
	this.handle.onmousedown = function(t) {
			t = t || window.event;
			RS.stopPropagation(t);
			e.contentDiv.className += e.UNSELECTABLE_CLASS;
			RS.enableSelection(!1);
			e.clientY = t.clientY;
			e.scrollTop = e.scrollDiv.scrollTop;
			e.mouseDown = !0
	};
	this.el.onmouseup = function() {
			e.setMouseUpAndEnableSelection(e)
	};
	RS.detectIE() ? this.el.onmouseleave = function() {
			e.setMouseUpAndEnableSelection(e)
	...
	this.el.onmousemove = function(t) {
			if (!e.mouseDown) return;
			t = t || window.event;
			e.scrollDiv.scrollTop = (t.clientY - e.clientY) / e.ratio + e.scrollTop
	};
	this.scrollbar.onclick = function(t) {
			t = t || window.event;
			RS.stopPropagation(t);
			var n = RS.getOffset(t) - e.handle.clientHeight / 2;
			e.scrollDiv.scrollTop = n / e.totalHandle * e.totalScrollable
	};
	this.el.onscroll = function(t) {
			t.preventDefault();
			e.el.scrollLeft = 0
	}
};
RS.RocketScroll.prototype.setMouseUpAndEnableSelection = function(e) {
	e.contentDiv.className = e.contentDiv.className.replace(e.UNSELECTABLE_CLASS, "");
	e.mouseDown = !1;
	clearTimeout(e.SELECTION_TIMEOUT);
	e.SELECTION_TIMEOUT = setTimeout(function() {
			RS.enableSelection(!0)
	}, 500)
};
RS.RocketScroll.prototype.refresh = function(e) {
	var t;
	e = e || !1;
	if (this.multiple) {
			for (t in this.elements) this.elements[t].refresh();
			return
	}
	this.scrollDiv.clientHeight > this.contentDiv.clientHeight ? this.scrollbar.style.display = "none" : this.scrollbar.style.display = "block";
...
RS.RocketScroll.prototype.refreshImages = function() {
	var e, t, n;
	e = RS.$("#" + this.el.id + " .scrollContent img");
	if (e.length > 0) {
			t = this;
			for (n = 0; n < e.length; n++) e.item(n).onload = function() {
					t.refresh();
					this.onload = null
			}
	}
